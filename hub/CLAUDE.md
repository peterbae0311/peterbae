...
* **배포(커밋 + push to main) 정책 — 반드시 지킬 것(2026-08-17 확정)**: 로컬에서 타입체크/빌드/실제 동작 검증까지 전부 끝난 뒤, 사용자가 배포를 명시적으로 요청("배포해줘", "커밋하고 배포해줘" 등)했을 때만 커밋하고 `main`에 push한다. 기능 구현이나 버그 수정을 마쳤다는 이유만으로 자동으로 커밋·push·배포하지 말 것 — 검증 완료와 배포 요청은 별개의 트리거이며, 후자가 있어야만 실행한다.
* **hub에 새 경로(페이지/API)를 추가할 때 실측으로 확인한 함정 3가지** (good-words 배포 때 발견, 2026-08-10):
  1. GitHub Actions의 `HUB_ENV` secret은 **빌드 시점에만** 쓰인다(`echo "$HUB_ENV" > .env` 후 `next build`). `remote-swap.sh`는 서버에 이미 있는 `~/apps/hub/.env`를 새 배포에도 그대로 복사해 유지하고, 빌드 아티팩트(tar)에는 `.env`가 아예 포함되지 않는다 — 즉 `HUB_ENV`를 갱신하고 재배포해도 서버에서 실제로 도는 프로세스의 환경변수는 안 바뀐다. 새 서버 전용 환경변수(Oracle 계정, LLM API 키 등)를 추가했다면 **SSH로 `~/apps/hub/.env`를 직접 수정하고 `pm2 reload hub --update-env`까지 해야** 반영된다.
  ...
  3. nginx의 `proxy_read_timeout` 기본값은 60초 — 외부 API(LLM 등)를 호출하는 라우트가 그보다 오래 걸리면 앱은 정상인데도 nginx가 먼저 504를 낸다. good-words의 `/api/good-words/generate`는 재시도 로직상 최악의 경우 최대 3분까지 걸릴 수 있어(`generate/route.ts`의 `MAX_ATTEMPTS` × `aiProviders.ts`의 provider별 타임아웃 × provider 수) 해당 `location` 블록에 `proxy_read_timeout 200s;`를 추가해뒀다. 오래 걸리는 외부 API를 호출하는 라우트를 새로 추가하면 이 타임아웃도 같이 검토할 것.
