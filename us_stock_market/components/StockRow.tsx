...
export default function StockRow({ stock, rank, exchangeRate, onStockClick }: StockRowProps) {
  ...
  return (
    ...
    >
      ...
      {/* 회사코드: 12px regular */}
      <span className="w-9 shrink-0 text-[12px] font-mono text-gray-200 ml-1">{stock.ticker}</span>

      {/* 회사명: 12px bold — 스캔 시 즉시 인식 */}
      <span className="flex-1 text-[12px] font-bold text-gray-300 truncate ml-1">{stock.nameKr}</span>

      {/* 종가($): 12px mono, medium weight — 참조 데이터 */}
      <span className="w-14 shrink-0 text-[12px] font-mono font-medium text-gray-200 text-right ml-1">
        {stock.closePrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
      </span>

      {/* 종가(₩): 12px mono, regular — 보조 참조 */}
      <span className="w-16 shrink-0 text-[12px] font-mono font-bold text-gray-400 text-right ml-3.5">
        {krw.toLocaleString('ko-KR')}
      </span>

      {/* 등락율: 12px mono bold — 핵심 정보, 색상으로도 강조 */}
      <span style={colorStyle} className="w-12 shrink-0 text-[12px] font-mono font-bold text-right ml-3.5">
        {sign}{stock.changePercent.toFixed(2)}%
      </span>

      {/* 등락액: 12px mono medium — 색상 연동, 보조 */}
      <span style={colorStyle} className="w-12 shrink-0 text-[12px] font-mono font-medium text-right ml-3.5">
