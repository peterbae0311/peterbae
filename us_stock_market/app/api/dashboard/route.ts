...
export async function GET() {
  try {
    const [exchangeRate, { data: tradeDates, error: datesError }] = await Promise.all([
      getExchangeRate().catch((): number => 1380),
      supabase
        .from('alert_sessions')
        .select('trade_date')
        .eq('status', 'success')
    ...
    if (uniqueDates.length === 0) return NextResponse.json({ data: [] });

    // Batch: fetch all sessions and all snapshots in 2 queries
    const { data: allSessions, error: sessionError } = await supabase
      .from('alert_sessions')
      .select('id, session_type, status, trade_date')
      .in('trade_date', uniqueDates)
      .in('status', ['success', 'skipped']);

    if (sessionError) throw sessionError;

    const successIds = (allSessions as SessionRow[] ?? [])
      .filter((s) => s.status === 'success')
      .map((s) => s.id);

    let allSnapshots: SectorSnapshotRow[] = [];
    if (successIds.length > 0) {
      const { data: snapshotRows, error: snapError } = await supabase
        .from('sector_snapshots')
        .select('id, session_id, sector_name, change_pct, rank, raw_data')
        .in('session_id', successIds)
        .order('rank', { ascending: true });
      if (!snapError) allSnapshots = (snapshotRows as SectorSnapshotRow[]) ?? [];
    }

    // Group in memory
    const snapshotsBySession = new Map<string, SectorSnapshotRow[]>();
    ...
    for (const sess of (allSessions as SessionRow[] ?? [])) {
      const arr = sessionsByDate.get(sess.trade_date) ?? [];
      arr.push(sess);
      sessionsByDate.set(sess.trade_date, arr);
    }

    const dayDataList: DayData[] = uniqueDates.map((tradeDate) => {
      const dateSessions = sessionsByDate.get(tradeDate) ?? [];
      const sessions: Session[] = [];

      // Deduplicate: keep only the first (earliest-inserted) success row per session_type.
      // Multiple rows with the same session_type+trade_date can exist when seed is run
      // multiple times. alert_sessions has no unique constraint on (trade_date, session_type).
      const seenSessionTypes = new Set<string>();

      for (const sessionRow of dateSessions) {
        if (sessionRow.status !== 'success') continue;
        if (seenSessionTypes.has(sessionRow.session_type)) continue;
        seenSessionTypes.add(sessionRow.session_type);

        const snaps = snapshotsBySession.get(sessionRow.id) ?? [];
        const sectors: Sector[] = snaps.map((snap) => {
