...
export default function CoverLetterPage() {
  ...
  const [interviewModels,     setInterviewModels]     = useState<Partial<Record<Difficulty, string>>>({});
  ...
  async function generateInterviewQs(difficulty: 'all' | Difficulty) {
    ...
    try {
      ...
      if (difficulty === 'all') {
        ...
        setInterviewModels(data.models ?? {});
      } else {
        setInterviewQs(prev => {
          const merged = [
        ...
        setInterviewModels(prev => ({ ...prev, ...data.models }));
      }
    } catch (err) {
  ...
  const selectedIQSapTerms = selectedIQ ? findSapTerms(selectedIQ.sample_answer) : [];
                          ...
                          <div className="flex items-center justify-between px-4 py-2.5 bg-white/40">
                            ...
                            <div className="flex items-center gap-1.5">
                              {interviewModels[diff] && (
                                <span className="text-[11px] text-gray-400" title="질문 생성에 사용된 모델">
                                  {interviewModels[diff]}
                                </span>
                              )}
                              <button
                                    ...
                                    <td className="px-4 py-1.5 text-gray-700">
                                      {q ? (
                                        <textarea
                                          ...
                                          className="block w-full bg-transparent text-sm text-gray-700 focus:outline-none leading-relaxed cursor-text resize-none overflow-hidden break-words"
                          ...
                          onClick={() => {
                            const text = [
                              ...
                              selectedIQSapTerms.length > 0
                                ? `[SAP 용어 설명]\n${selectedIQSapTerms.map(([term, desc]) => `• ${term}: ${desc}`).join('\n')}`
                                : '',
                              '',
                    ...
                    {selectedIQ ? (
                      <div className="space-y-5 text-sm">
                        <div>
                          ...
                          <p className="text-[16px] text-gray-700 leading-relaxed whitespace-pre-wrap">{selectedIQ.sample_answer}</p>
                        </div>
                        {selectedIQSapTerms.length > 0 && (
                          <>
                            ...
                            <div>
                              ...
                              <ul className="space-y-1">
                                {selectedIQSapTerms.map(([term, desc]) => (
                                  <li key={term} className="text-gray-600">
                                    <span className="font-medium text-gray-700">{term}</span>: {desc}
                                  </li>
                                ))}
                              </ul>
                            </div>
